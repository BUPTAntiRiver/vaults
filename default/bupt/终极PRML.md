...
最大熵模型，顾名思义，选择熵最大的参数作为最优结果。熵明明代表混乱度，为什么要选择一个熵最大的结果而不是熵最小呢？因为最大熵的原则是只根据已有的数据得出概率分布，不作其余预设。
于是我们可以将问题转换成一个优化问题：

$$
\begin{aligned}
\min_{P\in C} & -H(P)=\sum_{x,y}\tilde P(x)P(y|x)\log P(y|x)\\
\textrm{s.t.} & \quad \forall x,y: P(y|x) \geq 0 \\
& \quad \forall x:\sum_yP(y|x) = 1\\
&\quad E_P(f_i)=E_\tilde P(f_i), i=1,2,\ldots,n
\end{aligned}
$$

其中 $C$ 表示所有满足限制条件的概率分布，$\tilde P$ 表示观测结果。
$f_i$ 是特征函数，是人为设计出来的，用来代表我们想要从这些数据中获取哪些**输入与输出之间的关系**。
最后对偶化求解。
